...
async def get_current_user(request: Request):  #token: str = Depends(oath2_bearer)):
    try:
        ...
        if username is None or user_id is None:
            logout(request) #return None  #raise get_user_exception()
        return {"username": username, "id": user_id}
    except JWTError:
        raise HTTPException(status_code=404, detail="Not Found") #get_user_exception()
